...
function DeleteModal({ type, title, onDeleteBtnClick, setIsDeleteModalOpen }) {
	return (
		<div
			...
			className="fixed right-0 bottom-0 left-0 top-0 px-2 py-4 overflow-scroll scrollbar-hide z-50 justify-center items-center flex bg-[#00000080] dropdown"
		>
			<div className="scrollbar-hide overflow-y-scroll max-h-[95vh] my-auto bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md mx-auto  w-full px-8  py-8 rounded-xl ">
				<h3 className="font-bold text-red-500 text-xl">Delete {type} ?</h3>
				{type === "task" ? (
					<p className="text-gray-500 font-semibold -tracking-wide text-sm pt-6">
						Are you sure you want to delete the "{title}" taks and its subtasks?
						<br /> This action cannot be reversed.
					</p>
				) : (
					<p className="text-gray-500 font-semibold -tracking-wide text-sm pt-6">
						Are you sure you want to delete the "{title}" board?
						<br /> This action will remove all columns and tasks and cannot be reversed.
					</p>
				)}
